 ...
 *
 * // Set configuration for a specific provider and model
 * helper.setModelConfig('gpt-4', 'openai', {
 *   maxTokens: 8000,
 *   contextLength: 16000,
 *   temperature: 0.7,
 *   vision: true,
 ...
 * })
 *
 * // Get configuration (follows priority: user config > provider config > default config)
 * const config = helper.getModelConfig('gpt-4', 'openai')
 *
 * // Check if user has custom configuration
 * const hasCustom = helper.hasUserConfig('gpt-4', 'openai')
 *
 * // Get all configurations for a provider
 * const providerConfigs = helper.getProviderModelConfigs('openai')
 *
 * // Export for backup/sync
 * const backup = helper.exportConfigs()
 *
 * // Import from backup/sync
 * helper.importConfigs(backup, false)
 *
 * // Reset specific configuration
 * helper.resetModelConfig('gpt-4', 'openai')
 *
 * // Clear all configurations
 * helper.clearAllConfigs()
 */
